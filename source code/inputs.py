...
delTT = dict['delTT'][0]                # Ovearll Temperature rise [K]
...
nu = 1.8e-5                # Kinematic viscosity of fluid

#-----------------------------Stage Parameters---------------------------------
if bnumber > 1:
    for i in range(bnumber // 2 - 1, bnumber // 2):
    ...
    WorkRatio[-1] = 1 - np.sum(WorkRatio[0:-1])
else:
...
Bckswp = dict['Bckswp'][0]              # Backsweep angle [deg]
...
alphar = np.zeros((nstations, 1))
betar = np.zeros((nstations, 1))
M = np.zeros((nstations, 2))
...
I = np.zeros((nstations, 1))
h = np.zeros((nstations,1))
h0rel = np.zeros((nstations,1))
#===============================================================================
